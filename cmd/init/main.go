...
func main() {
	...
	fmt.Printf("🚀 MarkGo Blog Engine v%s - Quick Setup\n", version)
	fmt.Println("=====================================")
	fmt.Println()
...
func createSampleArticles(dir string, config BlogConfig) error {
	...
	if err := os.WriteFile(filepath.Join(articlesDir, "welcome.md"), []byte(welcomeContent), 0644); err != nil {
		return err
	}
	...
	return os.WriteFile(filepath.Join(articlesDir, "getting-started.md"), []byte(gettingStartedContent), 0644)
}
...
<body>
	...
	return os.WriteFile(filepath.Join(templatesDir, "base.html"), []byte(baseTemplate), 0644)
}
...
@media (max-width: 768px) {
	...
	return os.WriteFile(filepath.Join(cssDir, "style.css"), []byte(css), 0644)
}
...
func showHelp() {
	...
	fmt.Println()
	fmt.Printf("MarkGo v%s - The Developer's Blog Engine\n", version)
}
