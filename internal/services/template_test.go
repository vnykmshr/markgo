...
func BenchmarkTemplateService_Render(b *testing.B) {
	service := createBenchmarkTemplateService(b)

	data := map[string]any{
		"title":   "Benchmark Test",
		"content": "This is benchmark content for testing template rendering performance.",
	}

	for b.Loop() {
		var buf strings.Builder
		err := service.Render(&buf, "base.html", data)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func BenchmarkTemplateService_RenderToString(b *testing.B) {
	service := createBenchmarkTemplateService(b)
	...
	for b.Loop() {
		_, err := service.RenderToString("base.html", data)
		if err != nil {
			b.Fatal(err)
		}
	}
...
func createBenchmarkTemplateService(b *testing.B) *TemplateService {
	tempDir := b.TempDir()
	cfg := &config.Config{}

	// Create a template for benchmarking
